...
//Rare exceptions to indicate a bug in the _themoviedatabasesearch code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_themoviedatabasesearch = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_themoviedatabasesearch]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.themoviedatabasesearch.module"
